...
import { BTCRange, ETHRange, LogisticConst, ShibsRange, SupportedCoins, USDTRange } from "../const/Const";
...
export const formatTimezone = (time) => {
  let tz = timestampToLocalDate(time, "Z").slice(0, -3);
  if (tz.charAt(1) == 0) {
    return tz.replace("0", "");
  } else {
    return tz;
...
export const getPoolSizeLimit = (label) => {
  switch (label) {
    case SupportedCoins[0].label:
      return ETHRange.limit
    case SupportedCoins[1].label:
      return USDTRange.limit
    case SupportedCoins[2].label:
      return ShibsRange.limit
    case SupportedCoins[3].label:
      return USDTRange.limit
    case SupportedCoins[4].label:
      return BTCRange.limit
    default:
      return 0
  }
};

export const getPoolSizeMax = (label) => {
  switch (label) {
    case SupportedCoins[0].label:
      return ETHRange.max
    case SupportedCoins[1].label:
      return USDTRange.max
    case SupportedCoins[2].label:
      return ShibsRange.max
    case SupportedCoins[3].label:
      return USDTRange.max
    case SupportedCoins[4].label:
      return BTCRange.max
