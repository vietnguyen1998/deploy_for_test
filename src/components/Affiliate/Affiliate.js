...
const Affiliate = (props) => {
  ...
  const saveList = async () => {
    setLoading(true);
    try {
      //Check if an address ever created any Pool or made any play
      ...
      if (validAddrs.length != tempAddrList.length) {
        toast.error(
          "Some of your addresses were not added to the list, because they have already played or staked in Fuzanglong!"
        );
      }
    } catch (err) {
      setLoading(false);
    ...
    <Main reload={reload} loading={loading} setLoading={setLoading}>
      <div className="container body-section">
        ...
        <p className="grey mt-4">
          Affiliates are players whom you referred to Fuzanglong. Referrers will
          earn 1.25% commission for every winning earned by the referee (your
          affiliate). You may add up to 100 Affiliates. It costs 1 FZL token for
          each Affiliate added. At this moment, Fuzanglong allows a referrer to
            ...
            <div className="col-md-7">
              <Form.Group>
                <p className="black bold">
                  Become an affiliate. Deposit FZL to add referrals (1 FZL = 1
                  Referral):
                </p>
