...
import { getOdds, swapBetAmounts } from "../../utils/Utils";
...
const BettingPanel = (props) => {
  ...
  const betWithEth = () => {
    ...
    if (amount < pool.minBet) {
      return toast.error(
        "Betting amount should be equal or higher than minimum play"
      );
    }
    setLoading(true);
    let id = uuidv4();
      ...
      Erc20.approve(poolAddress, getWei(amount))
        .then((tx) => {
          tx.wait().then(() => {
            setLoading(false);
            setApproved(true);
            toast.success("Approved successfully! Can play now.");
          });
        })
        .catch((err) => {
          setLoading(false);
  ...
  return (
    <>
      ...
      <form className="grey mt-3">
        <span>
          {" "}
          Input {currency && currency.label} number. Minimum play is{" "}
          {pool.minBet}{" "}
        </span>
        <br />
      ...
      {pool.currency === ZeroAddress ? (
        ...
        >
          {!approved ? "Approve Token" : "Place Play"}
        </button>
      )}
    </>
  );
